...
const CATEGORIAS = [
  { id: "abaixo", max: 18.5, nome: "Abaixo do peso", texto: "Seu IMC ficou abaixo da faixa mais usada como referência para adultos." },
  { id: "normal", max: 25, nome: "Peso adequado", texto: "Seu IMC está na faixa considerada adequada para a maioria dos adultos." },
  { id: "sobrepeso", max: 30, nome: "Sobrepeso", texto: "Seu IMC ficou um pouco acima da faixa de referência." },
  { id: "ob1", max: 35, nome: "Obesidade grau I", texto: "Seu IMC está na faixa de obesidade grau I. Vale conversar com um profissional de saúde." },
  { id: "ob2", max: 40, nome: "Obesidade grau II", texto: "Seu IMC está na faixa de obesidade grau II. Um acompanhamento profissional ajuda bastante." },
  { id: "ob3", max: Infinity, nome: "Obesidade grau III", texto: "Seu IMC está na faixa mais alta da tabela. Procure orientação médica." }
];
...
function faixaPeso(altura) {
  ...
  return min.toFixed(1).replace(".", ",") + " a " + max.toFixed(1).replace(".", ",") + " kg";
}
...
form.addEventListener("submit", (evento) => {
  ...
  if (!peso || !alturaBruta || peso <= 0 || alturaBruta <= 0) {
    mostrarErro("Informe um peso e uma altura válidos.");
    return;
  }
  const altura = normalizarAltura(alturaBruta);
  if (altura < 0.5 || altura > 2.5) {
    mostrarErro("A altura parece incorreta. Use metros (1.70) ou centímetros (170).");
    return;
  }
  if (peso < 10 || peso > 400) {
  ...
  document.getElementById("imc-texto").textContent = categoria.texto + " Lembre-se: o IMC é só uma referência e não substitui uma avaliação profissional.";
